...
class ModelL2LossWithoutDropoutLReluAttentionPhonemeCluster(Model):  
    ...
    def build_model(self, num_classes, input_feature_dim, output_dir, logger=None):
        ...
        with self.graph.as_default():
            ...
            poster_temp=tf.gather(poster, indices=[7,18,13,15,32,29], axis=2)
            posterior=tf.concat((posterior,tf.expand_dims(tf.reduce_sum(poster_temp,axis=2),axis=2)),axis=2)
            ...
            poster_temp=tf.gather(poster, indices=[27,36,37], axis=2)
            posterior=tf.concat((posterior,tf.expand_dims(tf.reduce_sum(poster_temp,axis=2),axis=2)),axis=2)
...
class ModelL2LossWithoutDropoutLReluAttentionPhonemeCluster2layer(Model):  
    ...
    def build_model(self, num_classes, input_feature_dim, output_dir, logger=None):
        ...
        layer_sizes = [96, 96, 512, 512, 6 * 512]
        kernel_sizes = [5, 5, 7, 1, 1]
        embedding_sizes = [512, 512]
        beta = 0.0002
        mfcc_dim=23
...
class ModelL2LossWithoutDropoutLReluAttentionPhonemeClusterFig2a(Model):  #realization of SUSTDNN in Fig2(a)

    def __init__(self):
        super(ModelL2LossWithoutDropoutLReluAttentionPhonemeClusterFig2a, self).__init__()
    ...
    def build_model(self, num_classes, input_feature_dim, output_dir, logger=None):
        ...
        with self.graph.as_default():
            ...
            l2_loss = tf.constant(0.0)
            h_input = self.input_x[:, :, 0:mfcc_dim]
            poster=self.input_x[:, :, mfcc_dim:]
            ...
            for i, (kernel_size, layer_size) in enumerate(zip(kernel_sizes, layer_sizes)):
                if i == 0:
                    for j in range(num_phoneme):
                        with tf.variable_scope("frame_level_info_layer-%s_TDNN-%s" % (i,j)):
                            kernel_shape = [kernel_size, prev_dim, layer_size]
                            w = tf.Variable(tf.truncated_normal(kernel_shape, stddev=0.1), name="w")
                            b = tf.Variable(tf.constant(0.1, shape=[layer_size]), name="b")
                            conv = tf.nn.conv1d(h_input, w, stride=1, padding="SAME", name="conv-layer-%s-%s" % (i,j))
                            h = tf.nn.bias_add(conv, b)     # shape [None, None, layer_size]
                            
                            # Apply nonlinearity and BN
                            h = tf.nn.leaky_relu(h, alpha=0.2, name='lrelu')  #[None, None, layer_size]
                        
                            if j==0:
                                h_reshape=tf.expand_dims(h,axis=2)
                            else:
                                h_j=tf.expand_dims(h,axis=2)
                                h_reshape=tf.concat([h_reshape,h_j],axis=2)  #[None, None, num_phoneme, layer_size]
