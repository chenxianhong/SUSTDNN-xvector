...
int main(int argc, char *argv[]) {
  try {
    ...
    for (;!feat_reader.Done(); feat_reader.Next()) {
      ...
      MatrixIndexT mfcc_dim = 23;
      
      Matrix<BaseFloat> feat_23dim(feat.NumRows(), 
                                   mfcc_dim, kUndefined);
                                   
      Matrix<BaseFloat> cmvn_feat_23dim(feat.NumRows(), 
                                   mfcc_dim, kUndefined);
                                   
      feat_23dim = feat.ColRange(0, mfcc_dim);
                                  
                                  
      SlidingWindowCmn(opts, feat_23dim, &cmvn_feat_23dim);
      
      MatrixIndexT feat_stride=feat.Stride();
      MatrixIndexT cmvn_feat_23dim_stride=cmvn_feat_23dim.Stride();
      
      for (MatrixIndexT row = 0; row < feat.NumRows(); row++) {
          for (MatrixIndexT col = 0; col < mfcc_dim; col++) {
              feat.Data()[row*feat_stride+col] = cmvn_feat_23dim.Data()[row*cmvn_feat_23dim_stride+col];
          }
      }
